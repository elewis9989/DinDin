...
class Header extends PureComponent {
  constructor(props){
      ...
      this.state = {
        idPressed: this.props.selectedMonth,
        Months: [
  ...
  getItem(item) {
    ...
    this.props.updateSelectedMonth(item.id)
  }

  renderMonths({item}){
    return(
      <View style={styles.bottomContainer}>
        <Text style={[material.title, styles.monthText, {opacity: this.state.idPressed === item.id ? 1.0 : 0.3}]} onPress={()=> this.getItem(item)}>{item.value}</Text>
      </View>
    )
  }
    ...
    return (
      <View style={styles.container}>
        <View style={styles.topContainer}>
          <TouchableOpacity onPress={()=>{this.props.navigation.toggleDrawer()}} style={{padding: '4%'}}>
            <Image style={styles.menuIcon} source={require('../../assets/homescreen/sidemenu_btn.png')}/>
          </TouchableOpacity>
          <Text style={[material.title, styles.headerText]}> DinDin </Text>
